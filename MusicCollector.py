...
def load_collector():
    """open file and separate blank and non-blank lines, return list with two tuples"""
...
def new_album():
    """open file and save new positions"""
    new_position = []
...
def albums_by_artist(music):
    """Searching albums by artist if artist is on list"""
...
def albums_by_year(music):
    """Searching albums by year if year is on list"""
...
def musician_by_album(music):
    """Searching musician by album if album is on list"""
...
def albums_by_letter(music):
    """Searching albums by letter(s) if letter is on list"""
...
def albums_by_genre(music):
    """Searching albums by genre if genre is on list"""
...
def age_of_albums(music):
    """Function load current date from system, then current year and sum of modulo actions gives age of albums"""
    data = datetime.now()
    total_age = 0
    for age in music:
        if age[1][0] < 2018:
...
def random_album_by_genre(music):
    """Searching albums by genre if genre is on list and add to a new list, then draws one album by random.choice"""
    genre = input(D+'Choose genre: ').lower()
    albums = []
    for kind in music:
        if genre in kind[1][1]:
...
def amount_of_albums(music):
    """Searching number of albums for chosen artist by increasing value of variable 'counter' if album is valid """
...
def longest_album(music):
    """Searching the longest album by the longest string and boolean"""
    longest_time = '00:00'
    for lenght in music:
